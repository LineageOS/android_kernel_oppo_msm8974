 ...
 * Description: Interface to get device information.
 * History:
...
int register_device_proc(char *name, char *version, char *manufacture)
{
	...
	if (!parent) {
		...
		if (!parent) {
			pr_err("Failed to create devinfo proc\n");
			return -ENOENT;
		}
	}
	...
	d_entry = create_proc_read_entry(name, S_IRUGO, parent,
			device_read_proc, info);
	if (!d_entry) {
		pr_err("Failed to create %s proc\n", name);
		kfree(info);
		return -ENOENT;
	}
