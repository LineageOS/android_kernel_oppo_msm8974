...
int tfa98xx_restore_mtp(struct tfa98xx *tfa98xx)
{
    ...
    snd_soc_write(codec, 0x62, 0x8800);
    snd_soc_write(codec, 0x60, 0x0);
    snd_soc_write(codec, 0x62, 0x0);
...
static int tfa98xx_dsp_write_mem(struct tfa98xx *tfa98xx, unsigned int address, int value, int memtype)
{
	struct snd_soc_codec *codec = tfa98xx->codec;
	int ret = 0;
 ...
 * Disable DATAO
 */
 void tfa98xx_disabledatao(struct tfa98xx *tfa98xx)
{

    struct snd_soc_codec *codec = tfa98xx->codec;
	u16 status;

    status = snd_soc_read(codec, TFA98XX_I2SREG);
    status &= ~TFA98XX_I2SREG_I2SDOE;

    snd_soc_write(codec, TFA98XX_I2SREG, status);
}

/*
 * Enable DATAO
 */
 void tfa98xx_enabledatao(struct tfa98xx *tfa98xx)
{
...
int tfa98xx_coldboot(struct tfa98xx *tfa98xx, int state)
{
	...
	while (state == !tfa98xx_is_coldboot(tfa98xx)) {
		...
		ret = tfa98xx_dsp_write_mem(tfa98xx, CF_CONTROL, state, Tfa98xx_DMEM_IOMEM);
...
int tfa98xx_dsp_power_up(struct tfa98xx *tfa98xx)
{
    ...
    if (tries == 0) {
        ...
        /* NXP: if 0x84 is wrong, restore the correct i2cmtp settings */
        if (!mtp0)
        {
		    pr_err("mtp0 error.\n");
            tfa98xx_restore_i2cmtp(tfa98xx);
...
static u8 vsfwdelay_table[] = {
        ...
        0,0,0, /* Index 3 - Current/Volt Fractional Delay for 16KHz */
        0,0,0, /* Index 4 - Current/Volt Fractional Delay for 22KHz */
        0,0,0, /* Index 5 - Current/Volt Fractional Delay for 24KHz */
        0,0,0, /* Index 6 - Current/Volt Fractional Delay for 32KHz */
        0,0,0, /* Index 7 - Current/Volt Fractional Delay for 44KHz */
...
static int tfa9897_dsp_write_vsfwdelay_table(struct tfa98xx *tfa98xx)
{
        return tfa98xx_dsp_set_param(tfa98xx, MODULE_FRAMEWORK,
				FW_PARAM_SET_CURRENT_DELAY, sizeof(vsfwdelay_table),
				vsfwdelay_table);
}
...
{
        return tfa98xx_dsp_set_param(tfa98xx, MODULE_FRAMEWORK,
				FW_PARAM_SET_CURFRAC_DELAY, sizeof(cvfracdelay_table),
				cvfracdelay_table);
}
...
//    assert((otcOn == 0) || (otcOn == 1) );
	...
	snd_soc_write(codec, TFA98XX_MTP, (unsigned short)otcOn);
       //assert(err == Tfa98xx_Error_Ok);
...
int tfaRunSpeakerQuickBoost(struct tfa98xx *tfa98xx, int force)
{
	int ret = 0;
    //u32 re25;
	int done;
...
int tfa98xx_dsp_stop(struct tfa98xx *tfa98xx)
{
	...
	if (ret)
    {
        pr_err("tfa_error with power down");
    }
