...
#define EXP_FN_WORK_DELAY_MS 5	/* ms */
...
static int tp_power_on;
static int finger_up_state_flag;

int Lcd_type = 0;		// truly -- 0; jdi -- 1
...
static ssize_t synaptics_rmi4_open_or_close_holster_mode_store(struct device
							       *dev,
							       struct
							       device_attribute
							       *attr,
...
static ssize_t synaptics_rmi4_open_or_close_holster_mode_show(struct device
							      *dev,
							      struct
							      device_attribute
							      *attr, char *buf);
...
static ssize_t synaptics_rmi4_delta_data_show(struct device *dev,
					      struct device_attribute *attr,
					      char *buf);
static ssize_t synaptics_rmi4_show_register(struct device *dev,
					    struct device_attribute *attr,
					    char *buf);
static ssize_t synaptics_rmi4_show_interrupt(struct device *dev,
					     struct device_attribute *attr,
					     char *buf);
static ssize_t synaptics_rmi4_abs_delta_data_show(struct device *dev,
						  struct device_attribute *attr,
						  char *buf);
static ssize_t synaptics_rmi4_abs_raw_data_show(struct device *dev,
						struct device_attribute *attr,
						char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(show_register, S_IRUGO,
	       synaptics_rmi4_show_register,
	       synaptics_rmi4_store_error),
	__ATTR(show_interrupt, (S_IRUGO | S_IWUSR),
	       synaptics_rmi4_show_interrupt,
	       synaptics_rmi4_handle_interrupt),
	...
	__ATTR(flashprog, S_IRUGO,
	       ...
	       synaptics_rmi4_store_error),
	__ATTR(0 dbutton, (S_IRUGO | S_IWUSR),
	...
	__ATTR(delta_data, S_IRUGO,
	       ...
	       synaptics_rmi4_store_error),
	__ATTR(abs_delta_data, S_IRUGO,
	       synaptics_rmi4_abs_delta_data_show,
	       synaptics_rmi4_store_error),
	__ATTR(abs_raw_data, S_IRUGO,
	       synaptics_rmi4_abs_raw_data_show,
	       synaptics_rmi4_store_error),
					    ...
					    char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	unsigned char data[5];
	int retval = 0, status = 0, x = 0, y = 0;

	retval = synaptics_rmi4_i2c_read(rmi4_data, 0x06, data, sizeof(data));

	status = data[0];
	x = (data[2] << 8) | (data[1]);
	y = (data[4] << 8) | (data[3]);

	printk("The finger data : status = %d, (x,y) = (%d, %d)\n", status, x,
	       y);

	retval =
	    synaptics_rmi4_i2c_read(rmi4_data, 0x0e, &data[0], sizeof(data[0]));
	printk("Register 0x0e = 0x%x\n", data[0]);

	retval =
	    synaptics_rmi4_i2c_read(rmi4_data, 0x0f, &data[0], sizeof(data[0]));
	printk("Register 0x0f = 0x%x\n", data[0]);

	return snprintf(buf, PAGE_SIZE, "(x, y) = (0x%dx, 0x%dx)\n", x, y);
}
					     ...
					     char *buf)
{
	...
	irq_gpio = gpio_get_value(rmi4_data->irq_gpio);
							       ...
							       size_t count)
{
	...
	if (input == 1) {
		...
		synaptics_rmi4_i2c_write(rmi4_data,
					 rmi4_data->holstere_mode_control_addr,
					 (unsigned char *)&(rmi4_data->
							    holstere_mode_open_or_close),
					 1);
	} else if (input == 0) {
	...
	synaptics_rmi4_i2c_read(rmi4_data,
				rmi4_data->holstere_mode_control_addr, &databuf,
				1);
	printk("%s holstere mode %s\n", input ? "open" : "close",
	       (databuf ==
		(rmi4_data->holstere_mode_open_or_close)) ? "success" : "fail");
...
#define SYNA_ADDR_OBJECT_REPORT_FLAG 0x1d	//bit 0: finger ; bit 5 : glove
...
static void vk_calculate_area(void)
{
	int i;
	...
	for (i = 0; i < TP_VKEY_COUNT; ++i) {
		...
		printk("[syna][%d]w = %d,h = %d,x = %d,y = %d\n", i,
		       vkey_buttons[i].width, vkey_buttons[i].height,
		       vkey_buttons[i].x, vkey_buttons[i].y);
	}
			    ...
			    struct kobj_attribute *attr, char *buf)
{
	...
	len = sprintf(buf,
		      ...
		      ":%d:%d:%d:%d" "\n", VK_LCD_WIDTH / 2 + 0,
		      ts->vk_prop_center_y, ts->vk_prop_width,
		      ts->vk_prop_height, VK_LCD_WIDTH * 3 / 2,
		      ts->vk_prop_center_y, ts->vk_prop_width,
		      ts->vk_prop_height, VK_LCD_WIDTH * 5 / 2 + 20,
		      ts->vk_prop_center_y, ts->vk_prop_width,
		      ts->vk_prop_height);
...
static void synaptics_enable_gesture(struct synaptics_rmi4_data *rmi4_data,
				     bool enable)
{
	...
	synaptics_rmi4_i2c_read(rmi4_data, reportaddr, reportbuf,
				sizeof(reportbuf));
	...
	synaptics_rmi4_i2c_write(rmi4_data, reportaddr, reportbuf,
				 sizeof(reportbuf));
	//huqiao add for find7 to solve the screenoff gesture problme start
	synaptics_rmi4_i2c_read(rmi4_data, SYNA_ADDR_GESTURE_FLAG, &tmp_new, 1);
	if (tmp_new != 0x6b)
		synaptics_rmi4_i2c_write(rmi4_data, SYNA_ADDR_GESTURE_FLAG,
					 &reg, 1);
	//huqiao add for find7 to solve the screenoff gesture problme end
...
static int synaptics_enable_irqwake(struct synaptics_rmi4_data *rmi4_data,
				    bool enable)
{
	if (enable) {
				     ...
				     int count, int *eof, void *data)
{
	...
	print_ts(TS_ERROR,
		 "%s, gesturemode = %d! (%d:%d),(%d:%d),(%d:%d),(%d:%d),(%d:%d),(%d:%d),%d\n",
		 __func__, syna_rmi4_data->gesturemode,
		 syna_rmi4_data->points[0], syna_rmi4_data->points[1],
		 syna_rmi4_data->points[2], syna_rmi4_data->points[3],
		 syna_rmi4_data->points[4], syna_rmi4_data->points[5],
		 syna_rmi4_data->points[6], syna_rmi4_data->points[7],
		 syna_rmi4_data->points[8], syna_rmi4_data->points[9],
		 syna_rmi4_data->points[10], syna_rmi4_data->points[11],
		 syna_rmi4_data->points[12]);

	len =
	    sprintf(page, "%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d\n",
		    syna_rmi4_data->gesturemode, syna_rmi4_data->points[0],
		    syna_rmi4_data->points[1], syna_rmi4_data->points[2],
		    syna_rmi4_data->points[3], syna_rmi4_data->points[4],
		    syna_rmi4_data->points[5], syna_rmi4_data->points[6],
		    syna_rmi4_data->points[7], syna_rmi4_data->points[8],
		    syna_rmi4_data->points[9], syna_rmi4_data->points[10],
		    syna_rmi4_data->points[11], syna_rmi4_data->points[12]);

	return len;
}

static int synaptics_rmi4_gesture_proc_read(char *page, char **start, off_t off,
					    int count, int *eof, void *data)
{
	int len = 0;
	unsigned int enable;
...
static int synaptics_rmi4_gesture_proc_write(struct file *filp,
...
static int synaptics_rmi4_reg_proc_read(char *page, char **start, off_t off,
					int count, int *eof, void *data)
{
	int len;
	uint8_t tmp_new = 0;
	struct synaptics_rmi4_data *syna_ts_data = syna_rmi4_data;
	synaptics_rmi4_i2c_read(syna_ts_data, SYNA_ADDR_GESTURE_FLAG, &tmp_new,
				1);

	print_ts(TS_DEBUG, " tmp_new = 0x%x\n", tmp_new);
	len = sprintf(page, "%x\n", tmp_new);
...
static int synaptics_rmi4_reg_proc_write(struct file *filp,
					 const char __user * buff,
					 unsigned long len, void *data)
{
	uint8_t tmp_new = 0;
...
static void synaptics_rmi4_grove_enable(bool enable)
{
	...
	retval =
	    synaptics_rmi4_i2c_read(syna_rmi4_data,
				    SYNA_ADDR_OBJECT_REPORT_FLAG, &buf, 1);
	if (retval < 0) {
	...
	if (enable) {
		buf = buf | 0x20;
		synaptics_rmi4_i2c_write(syna_rmi4_data,
					 SYNA_ADDR_OBJECT_REPORT_FLAG, &buf,
					 sizeof(buf));
	} else {
		buf = buf & 0xdf;
		synaptics_rmi4_i2c_write(syna_rmi4_data,
...
static int synaptics_rmi4_proc_glove_write(struct file *filp,
					   const char __user * buff,
					   unsigned long len, void *data)
{
	...
	syna_rmi4_data->glove_enable = enable;

	print_ts(TS_INFO, "%s, glove enable = %x\n", __func__,
		 syna_rmi4_data->glove_enable);

	synaptics_rmi4_grove_enable(enable);

	return len;
}

//charge pump proc read function
static int synaptics_rmi4_charge_pump_read(char *page, char **start, off_t off,
					   int count, int *eof, void *data)
{
	int len = 0;
	uint8_t reg, charge_pump;

	synaptics_rmi4_i2c_read(syna_rmi4_data, F54_CTRL_BASE_ADDR + 25, &reg,
				1);

	charge_pump = (reg & 0x40) ? 1 : 0;

	print_ts(TS_INFO, "[syna]:reg = 0x%x, charge_pump = %d.\n", reg,
		 charge_pump);
...
//charge pump proc write function
static int synaptics_rmi4_charge_pump_write(struct file *filp,
					    const char __user * buff,
					    unsigned long len, void *data)
{
	uint8_t reg, charge_pump;
	...
	synaptics_rmi4_i2c_write(syna_rmi4_data, F54_CTRL_BASE_ADDR + 25, &reg,
				 1);
...
static int synaptics_rmi4_init_touchpanel_proc(void)
{
	...
	//for support tp2.0
	//proc_entry = create_proc_entry("charge_pump", 0666, procdir);
	if (proc_entry) {
		proc_entry->read_proc = synaptics_rmi4_charge_pump_read;
	...
	if (proc_entry) {
		proc_entry->write_proc = synaptics_rmi4_reg_proc_write;
		proc_entry->read_proc = synaptics_rmi4_reg_proc_read;
	}
	//huqiao add node for reading gesture register end
...
static inline void wait_test_cmd_finished(void)
{
	...
	do {
		mdelay(1);	//wait 1ms
		synaptics_rmi4_i2c_read(syna_rmi4_data,
					syna_rmi4_data->f54_cmd_base_addr,
					&data, 1);
		if (i <= 80) {
			i++;
		} else {
...
static int synaptics_rmi4_power_down_up_process(struct synaptics_rmi4_data
						*rmi4_data)
{
	int retval;
	...
	retval =
	    synaptics_rmi4_reset_device(rmi4_data,
					rmi4_data->f01_cmd_base_addr);
	if (retval < 0) {
		print_ts(TS_INFO, "%s, reset device failed !\n", __func__);
	...
	if (retval < 0) {
		print_ts(TS_INFO, "%s, reinit device failed !\n", __func__);
...
static int synaptics_set_int_mask(struct synaptics_rmi4_data *ts, int enable)
{
	...
	retval = synaptics_rmi4_i2c_read(ts, ts->f01_data_base_addr + 1, &buf, 1);	//clear the interrupt
	if (retval < 0) {
		print_ts(TS_INFO, "%s, I2c read error!\n", __func__);
	...
	if (retval < 0) {
		print_ts(TS_INFO, "%s, I2c write error!\n", __func__);
	}
...
static ssize_t synaptics_rmi4_read_delta_data(char *buf)
{
	...
	synaptics_rmi4_i2c_write(syna_ts_data, F54_DATA_BASE_ADDR + 1,
				 (unsigned char *)&word_value, 2);
		...
		for (y = 0; y < rx_num; y++) {
			ret =
			    i2c_smbus_read_word_data(client,
						     F54_DATA_BASE_ADDR + 3);
			read_data = ret & 0xffff;

			print_ts(TS_INFO, "%3d,", read_data);

			num_read_chars +=
			    sprintf(&(buf[num_read_chars]), "%d ", read_data);
		}
	}
...
static ssize_t synaptics_rmi4_read_abs_raw_data(char *buf)
{
	uint8_t tmp_new = 0;
	uint tx_num = 0;
	uint rx_num = 0;
...
#ifdef CONFIG_MACH_N3
	unsigned char data[(TX_NUM_WINTEK_N3 + RX_NUM_WINTEK_N3) * 4] = { 0 };
#else
	unsigned char data[(TX_NUM_WINTEK + RX_NUM_WINTEK) * 4] = { 0 };
#endif
	...
	synaptics_rmi4_i2c_read(syna_ts_data, F54_DATA_BASE_ADDR + 3, data,
				sizeof(data));

	for (x = 0; x < (tx_num + rx_num); x++) {
		delta_data =
		    ((int)data[4 * x]) | ((int)data[4 * x + 1]) << 8 | ((int)
									data[4 *
									     x +
									     2])
		    << 16 | ((int)data[4 * x + 3]) << 24;
		printk("abs_delta_data = %d\n", delta_data);

		num_read_chars +=
		    sprintf(&(buf[num_read_chars]), "%d ", delta_data);
	}
...
static ssize_t synaptics_rmi4_read_abs_delta_data(char *buf)
{
	uint8_t tmp_new = 0;
	uint tx_num = 0;
	uint rx_num = 0;
...
static ssize_t synaptics_rmi4_baseline_data_open_cbc(char *buf)
{
	...
	//enable cbc
	synaptics_rmi4_i2c_read(syna_ts_data, F54_CTRL_BASE_ADDR + 23, &tmp_old,
				1);
	tmp_new = tmp_old | 0x10;
	synaptics_rmi4_i2c_write(syna_ts_data, F54_CTRL_BASE_ADDR + 23,
				 &tmp_new, 1);

	// Forbid NoiseMitigation
	tmp_new = 0x01;
	synaptics_rmi4_i2c_write(syna_ts_data, F54_CTRL_BASE_ADDR + 7, &tmp_new,
				 1);
	tmp_new = 0x04;
	...
	for (x = 0; x < tx_num; x++) {
		...
		for (y = 0; y < rx_num; y++) {
			...
			print_ts(TS_DEBUG, "Raw[%2d][%2d] = %4d,  ", x, y,
				 read_data);
...
static ssize_t synaptics_rmi4_baseline_data_close_cbc(char *buf)
{
	...
	print_ts(TS_INFO,
		 "F54_DATA_BASE_ADDR = 0x%x, F54_CTRL_BASE_ADDR = 0x%x, F54_CMD_BASE_ADDR = 0x%x.\n",
	...
	synaptics_rmi4_i2c_write(syna_ts_data, F54_CTRL_BASE_ADDR + 20,
				 &tmp_new, 1);
	...
	synaptics_rmi4_i2c_read(syna_ts_data, F54_CTRL_BASE_ADDR + 25,
				&tmp_old2, 1);
	tmp_new = tmp_old2 & 0xdf;
	print_ts(TS_DEBUG, "tmp_old =0x%x ,tmp_new = 0x%x\n", tmp_old2,
		 tmp_new);
	synaptics_rmi4_i2c_write(syna_ts_data, F54_CTRL_BASE_ADDR + 25,
				 &tmp_new, 1);
...
static ssize_t synaptics_rmi4_baseline_data(char *buf, bool savefile)
{
	...
	if (isbaseline == true) {
		return sprintf(&(buf[num_read_chars]),
			       "-1 please wait to finish .\n");
	}
	...
	if (tx_num == 0 || rx_num == 0 || raw_cap_data == NULL
	    || rx2rx_lower_limit == 0 || rx2rx_upper_limit == 0
	    || tx_num > rx_num) {
		num_read_chars +=
		    sprintf(&(buf[num_read_chars]),
			    "%2d++ NO appropriate data for current tp.\n",
			    ++error_count);
		goto END_TP_TEST;
	}
	...
	print_ts(TS_DEBUG,
		 "-------------------Step 1: check baseline capacitance --------------------- \n");
	if (savefile) {
	...
	for (iCbcDataGroupe = 0; iCbcDataGroupe < 2; iCbcDataGroupe++) {
		if (0 == iCbcDataGroupe)	// enable cbc default, for enable cbc test
		{
			...
			synaptics_rmi4_i2c_write(syna_ts_data,
						 F54_DATA_BASE_ADDR, &tmp_new,
						 1);

			// Forbid NoiseMitigation
			tmp_new = 0x01;
			synaptics_rmi4_i2c_write(syna_ts_data,
						 F54_CTRL_BASE_ADDR + 7,
						 &tmp_new, 1);
			tmp_new = 0x04;
			synaptics_rmi4_i2c_write(syna_ts_data, F54_CMD_BASE_ADDR, &tmp_new, 1);	// force update
			wait_test_cmd_finished();
			...
			word_value = 0;	//set fifo 00 , first address
			synaptics_rmi4_i2c_write(syna_ts_data,
						 F54_DATA_BASE_ADDR + 1,
						 (unsigned char *)&word_value,
						 2);
			tmp_new = 0x01;	//send get report command
			synaptics_rmi4_i2c_write(syna_ts_data,
						 F54_CMD_BASE_ADDR, &tmp_new,
						 1);
			wait_test_cmd_finished();
		} else if (1 == iCbcDataGroupe)	// disable cbc,  for disable cbc test
		{
			if (iCbcDataSize < (tx_num * rx_num * 2)) {
				print_ts(TS_DEBUG,
					 " Only one group test data  \n");
				break;
			} else {
				cdata =
				    (int16_t *) raw_cap_data +
				    tx_num * rx_num * 2;
				...
				//disable cbc
				synaptics_rmi4_i2c_read(syna_ts_data,
							F54_CTRL_BASE_ADDR + 23,
							&tmp_old, 1);
				tmp_new = tmp_old & 0xef;
				synaptics_rmi4_i2c_write(syna_ts_data,
							 F54_CTRL_BASE_ADDR +
				...
				synaptics_rmi4_i2c_read(syna_ts_data,
							F54_CTRL_BASE_ADDR + 25,
							&tmp_old2, 1);
				tmp_new = tmp_old2 & 0xdf;
				print_ts(TS_DEBUG,
					 "tmp_old =0x%x ,tmp_new = 0x%x\n",
					 tmp_old2, tmp_new);
				synaptics_rmi4_i2c_write(syna_ts_data,
							 F54_CTRL_BASE_ADDR +
							 25, &tmp_new, 1);
...
#ifdef CONFIG_MACH_N3
		tx_judge_num = tx_num - 1;
		rx_judge_num = rx_num - 3;
#else
		tx_judge_num = tx_num;
		rx_judge_num = rx_num;
#endif
		for (x = 0; x < tx_num; x++) {
			for (y = 0; y < rx_num; y++) {
				...
				print_ts(TS_DEBUG, "Raw[%2d][%2d] = %4d,  ", x,
					 y, read_data);
				print_ts(TS_DEBUG, "range:[%4d ~ %4d], ",
					 cdata[0], cdata[1]);
				...
				if (fd >= 0) {
					...
					sys_write(fd, data_buf,
						  strlen(data_buf));
				}

				if (((x < tx_judge_num) && (y < rx_judge_num))
				    || ((x == tx_judge_num)
					&& (y >= rx_judge_num))) {
					if (read_data >= cdata[0]
					    && read_data <= cdata[1]) {
						...
						if (error_count >=
							...
							if (!savefile) {
								num_read_chars
								    ...
								    sprintf(&
									    (buf
									     [num_read_chars]),
									    "  == Reach max error count (%d), stop test.\n",
									    syna_test_max_err_count);
								goto END_TP_TEST;
							}
						} else {
							num_read_chars +=
							    sprintf(&
								    (buf
								     [num_read_chars]),
								    "%2d++ raw_cap[%02d][%02d]=%4d is not in range[%04d~%04d].\n",
								    ++error_count,
								    x, y,
								    read_data,
								    cdata[0],
	...
	print_ts(TS_DEBUG,
		 "-------------------Step 2 : high resistance test--------------------- \n");
	print_ts(TS_DEBUG, "n");

	tmp_new = 1;		// software reset TP
	synaptics_rmi4_i2c_write(syna_ts_data, syna_ts_data->f01_cmd_base_addr,
				 (unsigned char *)&tmp_new, 1);
	msleep(150);
	...
	for (i = 0; i < 3; i++) {
		...
		if (read_data >= iTemp[0] && read_data <= iTemp[1]) {
			...
			print_ts(TS_ERROR,
				 "========= High Resistance Raw NOT in range, Raw[%d] = %d \n ",
				 i, read_data);
			num_read_chars +=
			    sprintf(&(buf[num_read_chars]),
				    "=========== High Resistance Raw NOT in range, Raw[%d] = %d \n ",
	...
	y = synaptics_rmi4_i2c_read(syna_ts_data, F54_DATA_BASE_ADDR + 3,
				    data_buf, 7);
	print_ts(TS_DEBUG,
		 " trx-to-trx raw readback: %x  %x  %x  %x  %x  %x \n",
	...
	if (get_pcb_version() <= HW_VERSION__20)	//find7
	{
		if ((data_buf[0] == 0) && (data_buf[1] == 0)
			...
			if ((syna_ts_data->vendor_id == TP_VENDOR_TPK)
			    && (data_buf[4] == 0))
				print_ts(TS_DEBUG,
					 "tpk trx-to-trx test  tpk pass.\n");
			else if ((syna_ts_data->vendor_id == TP_VENDOR_WINTEK)
				 && ((data_buf[4] & 0xfe) == 0))
				print_ts(TS_DEBUG,
					 "wintek trx-to-trx test wintek pass.\n");
			else {
				if (syna_ts_data->vendor_id == TP_VENDOR_TPK) {
					print_ts(TS_ERROR,
						 " tpk trx-to-trx test error: data_buf[4] = %x   error \n",
						 data_buf[4]);
					num_read_chars +=
					    sprintf(&(buf[num_read_chars]),
						    "tpk  trx-to-trx test error: data_buf[4] = %x   error \n",
					...
					print_ts(TS_ERROR,
						 " wintek trx-to-trx test error: data_buf[4] & 0xfe  = %x   error \n",
						 data_buf[4] & 0xfe);
					num_read_chars +=
					    sprintf(&(buf[num_read_chars]),
						    "wintek  trx-to-trx test error: data_buf[4] & 0xfe  = %x   error \n",
			...
			print_ts(TS_ERROR,
				 " trx-to-trx test error: data_buf[3]&0x0f =%x data_buf[5]&0x7f=%x \n",
	...
	} else			//find7s
	{
		if ((data_buf[0] == 0) && (data_buf[1] == 0)
		    && (data_buf[2] == 0)
		    && ((data_buf[3] & 0x07) == 0) && (data_buf[4] == 0)
		    && ((data_buf[5] & 0x3f) == 0)) {
			print_ts(TS_DEBUG, " trx-to-trx test pass.\n");
		} else {
			print_ts(TS_ERROR,
				 " trx-to-trx test error: data_buf[3]&0x07 =%x data_buf[5]&0x3f=%x \n",
	...
	if (((data_buf[0] & 0x7e) == 0) && ((data_buf[1] & 0x7f) == 0)
	    && ((data_buf[2] & 0xfb) == 0)
	    && (data_buf[3] == 0) && ((data_buf[4] & 0xfe) == 0)
	    && (data_buf[5] == 0)) {
		print_ts(TS_DEBUG, " trx-to-trx test pass.\n");
	} else {
		print_ts(TS_ERROR,
	...
	y = synaptics_rmi4_i2c_read(syna_ts_data, F54_DATA_BASE_ADDR + 3,
				    data_buf, 7);
	print_ts(TS_DEBUG,
		 "trx-to-ground raw readback: %x  %x  %x  %x  %x  %x \n",
		 data_buf[0], data_buf[1], data_buf[2], data_buf[3],
		 data_buf[4], data_buf[5]);
	...
	if (get_pcb_version() <= HW_VERSION__20)	//find7
	{
		if ((data_buf[0] == 0xff) && (data_buf[1] == 0xff)
		    && (data_buf[2] == 0xff)
		    && ((data_buf[3] & 0x0f) == 0x0f)
		    && ((data_buf[5] & 0x7f) == 0x7f)) {
			if ((syna_ts_data->vendor_id == TP_VENDOR_TPK)
			    && (data_buf[4] == 0xff))
				print_ts(TS_DEBUG, "pass.\n");
			else if ((syna_ts_data->vendor_id == TP_VENDOR_WINTEK)
				 && ((data_buf[4] & 0xfe) == 0xfe))
				print_ts(TS_DEBUG, "pass.\n");
			else {
				print_ts(TS_ERROR,
					 "%d: syna_ts_data->vendor_id=%d,data_buf[4]=%d\n",
					 __LINE__, syna_ts_data->vendor_id,
					 data_buf[4]);
				print_ts(TS_ERROR,
					 " trx-to-ground  Test Failed [%d]\n",
					 __LINE__);
				num_read_chars +=
				    sprintf(&(buf[num_read_chars]),
					    " trx-to-ground  Test Failed [%d]\n",
	...
	} else			//find7s
	{
		if ((data_buf[0] == 0xff) && (data_buf[1] == 0xff)
		    && (data_buf[2] == 0xff)
		    && ((data_buf[3] & 0x07) == 0x7) && (data_buf[4] == 0xff)
		    && ((data_buf[5] & 0x3f) == 0x3f)) {
			print_ts(TS_DEBUG, "pass.\n");
		} else {
			print_ts(TS_ERROR,
				 "%d: syna_ts_data->vendor_id=%d,data_buf[4]=%d\n",
...
#elif defined(CONFIG_MACH_N3)
	if (syna_ts_data->vendor_id == TP_VENDOR_WINTEK) {
		if (((data_buf[0] & 0x7e) == 0x7e)
		    && ((data_buf[1] & 0x7f) == 0x7f)
		    && ((data_buf[2] & 0xfb) == 0xfb)
		    && (data_buf[3] == 0xff) && ((data_buf[4] & 0xfe) == 0xfe)
		    && (data_buf[5] == 0xff)) {
			print_ts(TS_DEBUG, "pass.\n");
		} else {
			print_ts(TS_ERROR,
		...
		if ((data_buf[0] == 0xff) && (data_buf[1] == 0xff)
		    && ((data_buf[2] & 0x3a) == 0x3a)
		    && ((data_buf[3] & 0x7e) == 0x7e)
		    && ((data_buf[4] & 0xbd) == 0xbd)
		    && ((data_buf[5] & 0x7e) == 0x7e)) {
			print_ts(TS_DEBUG, "pass.\n");
		} else {
			print_ts(TS_ERROR,
				 "%d: syna_ts_data->vendor_id=%d,data_buf[4]=%d\n",
		...
		if (((data_buf[0] & 0xee) == 0xee)
		    && ((data_buf[1] & 0xfb) == 0xfb)
		    && ((data_buf[2] & 0xaa) == 0xaa)
		    && ((data_buf[3] & 0xfd) == 0xfd)
		    && ((data_buf[4] & 0xf9) == 0xf9)
		    && ((data_buf[5] & 0xff) == 0xff)) {
			print_ts(TS_DEBUG, "pass.\n");
		} else {
			print_ts(TS_ERROR,
				 "%d: syna_ts_data->vendor_id=%d,data_buf[4]=%d\n",
		...
		if ((data_buf[0] == 0xff) && ((data_buf[1] & 0xdf) == 0xdf)
		    && ((data_buf[2] & 0x3a) == 0x3a)
		    && ((data_buf[3] & 0x7e) == 0x7e)
		    && ((data_buf[4] & 0xbd) == 0xbd)
		    && ((data_buf[5] & 0x7e) == 0x7e)) {
...
END_TP_TEST:
	...
	printk(error_count ? " **********  TP auto  test error  ********** " :
	       " **********  TP All test passed ********** ");
	num_read_chars +=
	    sprintf(&(buf[num_read_chars]), "imageid=0x%x,deviceid=0x%x\n",
		    syna_ts_data->image_cid, syna_ts_data->device_cid);

	num_read_chars +=
	    sprintf(&(buf[num_read_chars]), "%d error(s). %s\n", error_count,
		    error_count ? "" :
		    " **********   All test passed. ********** ");
	synaptics_rmi4_reset_device(syna_ts_data,
				    syna_ts_data->f01_cmd_base_addr);
...
static ssize_t synaptics_rmi4_abs_delta_data_show(struct device *dev,
						  ...
						  char *buf)
{
	return synaptics_rmi4_read_abs_delta_data(buf);
}
...
static ssize_t synaptics_rmi4_abs_raw_data_show(struct device *dev,
						...
						char *buf)
{
	return synaptics_rmi4_read_abs_raw_data(buf);
}
...
static void synaptics_init_gpio(struct synaptics_rmi4_data *ts)
{
	struct gpio synaptics_all_gpio[] = {
		{ts->irq_gpio, GPIOF_DIR_IN, "synaptics_irq_gpio"},
		{ts->reset_gpio, GPIOF_OUT_INIT_HIGH, "synaptics_reset_gpio"},
		{ts->wakeup_gpio, GPIOF_DIR_IN, "synaptics_wakeup_gpio"},
		{ts->id_gpio, GPIOF_DIR_IN, "synaptics_id_gpio"},
	};

	gpio_request_array(synaptics_all_gpio, ARRAY_SIZE(synaptics_all_gpio));

	gpio_tlmm_config(GPIO_CFG
			 (ts->irq_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
			  GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG
			 (ts->reset_gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
			  GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG
			 (ts->wakeup_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
			  GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG
			 (ts->id_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
			  GPIO_CFG_2MA), GPIO_CFG_ENABLE);
			      ...
			      struct synaptics_rmi4_data *ts)
{
	...
	if (dev->of_node) {
		...
		/* reset, irq gpio info */
		ts->irq_gpio = of_get_named_gpio(np, "synaptics,irq-gpio", 0);	//gpio 61
		ts->reset_gpio = of_get_named_gpio(np, "synaptics,reset-gpio", 0);	//gpio 60
		...
		if (!chosen_node) {
			printk("%s: get chosen node failed\n", __func__);
		}

		cmd_line = of_get_property(chosen_node, "bootargs", &len);
		if (!cmd_line) {
			printk("%s: get bootargs failed\n", __func__);
		}

		cmd_len = strlen(cmd_line);
		disp_idx =
		    strnstr(cmd_line, "qcom,mdss_dsi_truly_1080p_cmd", cmd_len);
...
static unsigned char synaptics_rmi4_update_gesture(unsigned char *gesture,
						   unsigned char *gestureext)
{
	...
	for (i = 0; i < 2 * 6; i++) {
		points[i] =
		    ((unsigned short)gestureext[i * 2]) |
		    (((unsigned short)gestureext[i * 2 + 1]) << 8);
	}

	print_ts(TS_ERROR,
		 "%s, gestureext[24] = 0x%x! (%d:%d),(%d:%d),(%d:%d),(%d:%d)\n",
		 __func__, gestureext[24], points[0], points[1], points[2],
		 points[3], points[4], points[5], points[6], points[7]);

	points[i] = (gestureext[24] == 0x10) ? 1 : (gestureext[24] == 0x20) ? 0 : 2;	// 1--clockwise, 0--anticlockwise, not circle, report 2

	print_ts(TS_ERROR,
		 KERN_ERR "%s, gesture[0] = 0x%x, points[12] = %d (%s)\n",
		 __func__, gesture[0], points[12],
		 (points[12] == 1) ? "clockwise" : (points[12] ==
						    0) ? "anticlockwise" :
		 "not circle");
	switch (gesture[0]) {
	...
	case SYNA_TWO_FINGER_SWIPE:
		gesturemode =
		    ...
		    (gestureext[24] == 0x80) ? DouSwip : UnkownGestrue;

		if (gesturemode == DouSwip ||
		    gesturemode == Down2UpSwip || gesturemode == Up2DownSwip) {
	...
	if (gesturemode != UnkownGestrue) {
		...
		memcpy(syna_rmi4_data->points, points,
		       sizeof(syna_rmi4_data->points));
	} else {
		keyvalue = 0;
	}

	print_ts(TS_ERROR,
		 "%s, gesture keyvalue = %d, gesturemode = %d (%s).\n",
		 __func__, keyvalue, gesturemode,
		 (gesturemode == 0) ? "UnkownGestrue" : (gesturemode ==
							 1) ? "Double Tap"
		 : (gesturemode == 2) ? "V" : (gesturemode ==
					       3) ? "^" : (gesturemode ==
							   4) ? ">"
		 : (gesturemode == 5) ? "<" : (gesturemode ==
					       6) ? "O" : (gesturemode ==
							   7) ? "||"
		 : (gesturemode == 8) ? "-->" : (gesturemode ==
						 9) ? "<--" : (gesturemode ==
							       10) ? "|V"
		 : (gesturemode == 11) ? "|^" : (gesturemode ==
						 12) ? "M" : "W");
					 ...
					 struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->gesture) {
		retval =
		    synaptics_rmi4_i2c_read(rmi4_data, SYNA_ADDR_GESTURE_OFFSET,
					    gesture, sizeof(gesture));

#ifdef CONFIG_MACH_N3
		retval =
		    synaptics_rmi4_i2c_read(rmi4_data,
					    SYNA_ADDR_GESTURE_EXT_S3528,
					    gestureext, sizeof(gestureext));
#else
		retval =
		    synaptics_rmi4_i2c_read(rmi4_data,
					    SYNA_ADDR_GESTURE_EXT_S3508,
					    gestureext, sizeof(gestureext));
#endif
		...
		if (gesture[0]) {
			keyvalue =
			    synaptics_rmi4_update_gesture(gesture, gestureext);
			if (keyvalue) {
				keyvalue = KEY_F4;

				print_ts(TS_ERROR,
					 KERN_ERR "report gesture keyvalue!\n");
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status) {
...

			{
				...
				//print_ts(TS_DEBUG, "%s: virtual_key = %d\n", __func__, pressed_vkey);
			...
			input_mt_report_slot_state(rmi4_data->input_dev,
						   ...
						   finger_status);
			input_report_key(rmi4_data->input_dev, BTN_TOUCH, 1);
			input_report_key(rmi4_data->input_dev, BTN_TOOL_FINGER,
					 1);
			input_report_abs(rmi4_data->input_dev,
					 ABS_MT_POSITION_X, x);
			input_report_abs(rmi4_data->input_dev,
			...
			print_ts(TS_DEBUG,
				 "Finger %2d: status = 0x%02x, x = %4d, y = %4d\n",
				 finger, finger_status, x, y);

			touch_count++;
			finger_info |= 1;

			finger_up_state_flag = 0;
		}
	}

	for (finger = 0; finger < fingers_to_process; finger++) {
		finger_status =
		    (finger_info >> (fingers_to_process - finger - 1)) & 1;
		if (!finger_status) {
			input_mt_slot(rmi4_data->input_dev, finger);
			input_mt_report_slot_state(rmi4_data->input_dev,
						   MT_TOOL_FINGER,
	...
	if ((touch_count == 0) && (finger_up_state_flag == 0)) {
		finger_up_state_flag = 1;
				      ...
				      struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		dev_dbg(&rmi4_data->i2c_client->dev,
			"%s: Button %d (code %d) ->%d\n",
			__func__, button, f1a->button_map[button], status);
#ifdef NO_0D_WHILE_2D
					...
					struct synaptics_rmi4_fn *fhandler)
{
	...
	//print_ts(TS_DEBUG,"%s: Function %02x reporting\n",__func__, fhandler->fn_number);

	switch (fhandler->fn_number) {
	case SYNAPTICS_RMI4_F11:
		touch_count_2d =
		    synaptics_rmi4_f11_abs_report(rmi4_data, fhandler);
		if (touch_count_2d)
...
static void tpd_down(struct synaptics_rmi4_data *rmi4_data, int raw_x,
		     int raw_y, int x, int y, int p)
{
	if (rmi4_data && rmi4_data->input_dev) {
		input_mt_slot(rmi4_data->input_dev, 0);
		input_mt_report_slot_state(rmi4_data->input_dev, MT_TOOL_FINGER,
					   1);
		...
		input_report_abs(rmi4_data->input_dev, ABS_MT_WIDTH_MAJOR,
				 (raw_x + raw_y) / 2);
...
static void tpd_up(struct synaptics_rmi4_data *rmi4_data, int raw_x, int raw_y,
		   int x, int y, int p)
{
	if (rmi4_data && rmi4_data->input_dev) {
		input_mt_slot(rmi4_data->input_dev, 0);
...
static void int_key_report(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if ((ret & 0x07) != 0) {
		...
		if (ret & 0x01)	//menu
		{
			tpd_down(rmi4_data, 40, 20, rmi4_data->sensor_max_x / 6,
				 VK_VENTE_Y, 44);
		}

		if (ret & 0x02)	//home
		{
			tpd_down(rmi4_data, 40, 20, rmi4_data->sensor_max_x / 2,
				 VK_VENTE_Y, 44);
		}

		if (ret & 0x04)	//reback
		{
			tpd_down(rmi4_data, 40, 20,
				 rmi4_data->sensor_max_x * 5 / 6, VK_VENTE_Y,
				 44);
		}
	} else {
...
static void synaptics_rmi4_sensor_report(struct synaptics_rmi4_data *rmi4_data)
{
	 ...
	 */
	retval =
	    synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_data_base_addr,
				    data, rmi4_data->num_of_intr_regs + 1);
	if (retval < 0) {
		print_ts(TS_ERROR, "%s:I2c transfer error,  esd process !\n",
			 __func__);

		synaptics_rmi4_power_down_up_process(rmi4_data);	// zhangqiang add for tp esd process
	...
	print_ts(TS_DEBUG, "%s: data[1] = 0x%x\n", __func__, data[1]);

	if (data[1] & 0x04) {
		/*
		 * Traverse the function handler list and service the source(s) of the interrupt accordingly.
		 */
		if (!list_empty(&rmi->support_fn_list)) {
			list_for_each_entry(fhandler, &rmi->support_fn_list,
					    link) {
				if (list_empty(&rmi->support_fn_list)) {
					printk(KERN_ERR
					       "[TP] %s:%d support_fn_list is empty, tp happen erro,will reset tp\n",
					       __func__, __LINE__);
					up(&work_sem);
					return;
				}
	...
	if (data[1] & 0x10) {
		if (rmi4_data->gesture)	// tp gesture , double tap virtual key area
		{
			retval =
			    synaptics_rmi4_i2c_read(rmi4_data,
						    SYNA_ADDR_F1A_0D_DATA02,
						    &gesture_0d,
						    sizeof(gesture_0d));

			print_ts(TS_ERROR,
				 "Gesture Double tap (gesture_0d = 0x%x) \n",
				 gesture_0d);
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry(exp_fhandler, &exp_data.list, link) {
			if (exp_fhandler == NULL) {
				printk
				    ("[TP] %s:%d  fhandler is NULL, continue loop\n",
				     __func__, __LINE__);
				continue;
			}
				     ...
				     bool enable)
{
	...
	if (enable) {
		...
		retval =
		    request_threaded_irq(rmi4_data->irq, NULL,
					 synaptics_rmi4_irq,
					 platform_data->
					 irq_flags | IRQF_ONESHOT, DRIVER_NAME,
					 rmi4_data);
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
				       ...
				       bool * was_in_bl_mode)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
					 rmi4_data->f01_data_base_addr + 1,
					 &intr_status, sizeof(intr_status));
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
			"%s: Failed to read interrupt status\n", __func__);
		return retval;
...
static int synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval =
	    synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_ctrl_base_addr,
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				retval =
				    synaptics_rmi4_check_status(rmi4_data,
								&was_in_bl_mode);
				if (retval < 0) {
					dev_err(&rmi4_data->i2c_client->dev,
			...
			case SYNAPTICS_RMI4_F11:
				...
				retval =
				    synaptics_rmi4_alloc_fh(&fhandler, &rmi_fd,
							    page_number);
				if (retval < 0) {
					dev_err(&rmi4_data->i2c_client->dev,
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_POSITION_X,
			     rmi4_data->snap_left,
			     rmi4_data->sensor_max_x - rmi4_data->snap_right, 0,
			     0);
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_POSITION_Y,
			     rmi4_data->snap_top,
			     rmi4_data->sensor_max_y - rmi4_data->snap_bottom,
			     0, 0);
#else
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_POSITION_X,
			     rmi4_data->snap_left,
	...
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_TOUCH_MINOR, 0,
			     rmi4_data->max_touch_width, 0, 0);
#endif
...
static int synaptics_rmi4_reinit_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	print_ts(TS_INFO, "%s : F54_CMD_BASE_ADDR = 0x%x \n", __func__,
		 F54_CMD_BASE_ADDR);
				       ...
				       unsigned short f01_cmd_base_addr)
{
	...
	synaptics_rmi4_irq_enable(rmi4_data, true);
	synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_data_base_addr + 1, (unsigned char *)&temp, 1);	// clear interrupt
...
static void synaptics_rmi4_exp_fn_work(struct work_struct *work)
{
	...
	mutex_lock(&exp_data.mutex);
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry_safe(exp_fhandler, exp_fhandler_temp,
					 &exp_data.list, link) {
			if (exp_fhandler == NULL) {
				printk
				    ("[TP] %s:%d  fhandler is NULL, continue loop\n",
...
exit:
	...
	if (exp_data.queue_work) {
		queue_delayed_work(exp_data.workqueue, &exp_data.work,
...
static void synaptics_rmi4_get_vendorid(struct synaptics_rmi4_data *rmi4_data)
{
	...
	print_ts(TS_INFO,
		 KERN_ERR "%s, id_gpio(%d) = %d, wakeup_gpio(%d) = %d\n",
		 __func__, rmi4_data->id_gpio, id_gpio, rmi4_data->wakeup_gpio,
		 wakeup_gpio);

#ifdef CONFIG_MACH_FIND7
	if (get_pcb_version() >= HW_VERSION__21)
		vendor_id =
		    synaptics_rmi4_get_vendorid1(id_gpio, wakeup_gpio, 0);
	else
		vendor_id =
		    synaptics_rmi4_get_vendorid2(id_gpio, wakeup_gpio, 0);
	print_ts(TS_INFO, KERN_ERR "[syna_13077] vendor id: %x\n", vendor_id);
#elif defined(CONFIG_MACH_N3)
	vendor_id = synaptics_rmi4_get_vendorid3(id_gpio, wakeup_gpio, 0);
	print_ts(TS_INFO, KERN_ERR "[syna_14021] vendor id: %x\n", vendor_id);
#endif
...
#ifdef CONFIG_FB
static int fb_notifier_callback(struct notifier_block *p, unsigned long event,
				void *data)
{
	...
	case FB_EVENT_BLANK:
		...
		if (new_status != UNBLANK) {
			...
			synaptics_rmi4_suspend(&
					       (syna_rmi4_data->input_dev->
						dev));
		} else {
			print_ts(TS_DEBUG, KERN_ERR "[syna]:resume tp\n");
			synaptics_rmi4_resume(&
					      (syna_rmi4_data->input_dev->dev));
		}
					  ...
					  const struct i2c_device_id *dev_id)
{
	...
	print_ts(TS_INFO, "%s, tp probe start\n", __func__);
	...
	if ((get_boot_mode() == MSM_BOOT_MODE__FACTORY ||
		...
		printk
		    ("TP detected boot mode is ftm,not need tp,need  power off 8941_l22 \n");
	...
	print_ts(TS_INFO, "%s, get_pcb_version = %d\n", __func__,
		 get_pcb_version());
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		retval =
		    sysfs_create_file(&rmi4_data->input_dev->dev.kobj,
	...
	synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_ctrl_base_addr + 1,
				&int_mask, 1);
	print_ts(TS_INFO, "%s: Interrupt enable mask = 0x%x\n", __func__,
		 int_mask);

	print_ts(TS_INFO, "%s, tp probe end\n", __func__);
...
err_regulator:
	...
	print_ts(TS_INFO, "%s, tp probe failed !\n", __func__);
...
static int __devexit synaptics_rmi4_remove(struct i2c_client *client)
{
	...
	if (platform_data->gpio_config) {
		...
		if (platform_data->reset_gpio >= 0) {
			platform_data->gpio_config(platform_data->reset_gpio,
						   false);
		}
	}
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (platform_data->regulator_en) {
		if (tp_power_on == 1) {
			...
			print_ts(TS_INFO, "%s tp sleep, close tp's vdd !\n",
				 __func__);
			regulator_disable(rmi4_data->regulator);
		}
	}
...
static void speedup_synaptics_resume(struct work_struct *work)
{
	...
	retval = synaptics_set_int_mask(syna_rmi4_data, 0);	//mingqiang.guo add for black gesture failure
	if (retval < 0) {
		print_ts(TS_INFO, "%s: Failed to set int mask !\n", __func__);

		print_ts(TS_INFO, "[TP] %s out!\n", __func__);
	...
	//software reset it
	synaptics_rmi4_i2c_write(rmi4_data, rmi4_data->f01_cmd_base_addr, &val,
				 sizeof(val));
	msleep(20);
	synaptics_rmi4_reinit_device(rmi4_data);

	print_ts(TS_INFO, "%s: f01_cmd_base_addr = 0x%x!\n", __func__,
		 rmi4_data->f01_cmd_base_addr);
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (platform_data->regulator_en) {
		...
		if (tp_power_on == 0) {
			...
			print_ts(TS_INFO, "%s tp resume, open tp's vdd !\n",
				 __func__);
			regulator_enable(rmi4_data->regulator);
	...
	queue_delayed_work(syna_rmi4_data->speedup_resume_wq,
			   &syna_rmi4_data->speed_up_work,
			   msecs_to_jiffies(10));

	print_ts(TS_DEBUG, "%s is called over \n", __func__);
...
static const struct dev_pm_ops synaptics_i2c_dev_pm_ops = {
