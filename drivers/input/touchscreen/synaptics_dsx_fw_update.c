...
static int fwu_read_f34_queries(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   ...
				   BOOTLOADER_ID_OFFSET, fwu->bootloader_id,
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->f34_fd.query_base_addr +
				   fwu->properties_off, &fwu->flash_properties,
			    ...
			    unsigned char command)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
				    fwu->f34_fd.data_base_addr +
				    BLOCK_NUMBER_OFFSET, block_offset,
				    sizeof(block_offset));
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static int fwu_write_lockdown(void)
{
	return fwu_write_blocks((unsigned char *)fwu->lockdown_data,
				fwu->lockdown_block_count,
				CMD_WRITE_LOCKDOWN_BLOCK);
}
...
static int fwu_write_bootloader_id(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
				    fwu->f34_fd.data_base_addr +
				    fwu->blk_data_off, fwu->bootloader_id,
				    sizeof(fwu->bootloader_id));
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
...
static int fwu_start_write_config(void)
{
	...
	if ((fwu->config_area == UI_CONFIG_AREA) &&
		...
		if (header.config_size) {
			fwu->config_data = fwu->ext_data_source +
			    FW_IMAGE_OFFSET + header.firmware_size;
		} else {
			return -EINVAL;
		}
	}
...
static unsigned char *fwu_rmi4_get_firmware_data(void)
{
	...
	if (fwu->image_name && fwu->image_name[0] != 0)	//add manual update firmware
		...
		if (get_pcb_version() >= HW_VERSION__21)
			firmwaredata =
			    (unsigned char *)Syna_Firmware_Data_tpk_find7s;
		else
		...
		if (Lcd_type == 0)
			firmwaredata =
			    (unsigned char *)Syna_Firmware_Data_Truly;
		else
			firmwaredata =
			    (unsigned char *)Syna_Firmware_Data_Truly_JDI;
	}
	...
	if (temp[0] == '1') {
		...
		if (!fwu->image_name[0]) {
			...
			queue_delayed_work(fwu->fwu_workqueue,
					   &fwu->fwu_work, msecs_to_jiffies(5));
		}
...
static int init_synaptics_proc(void)
{
	...
	struct proc_dir_entry *proc_entry =
	    create_proc_entry("syna_write", 0666, NULL);

	if (proc_entry == NULL) {
		ret = -ENOMEM;
		printk(KERN_INFO
		       "init_synaptics_proc: Couldn't create proc entry\n");
	} else {
