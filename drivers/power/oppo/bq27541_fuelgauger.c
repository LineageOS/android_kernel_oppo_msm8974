...
static bool bq27541_authenticate(struct i2c_client *client)
{
	...
	for (i = 0; i < 20; i++)
		pr_info("send_buf[%d]:0x%x,recv_buf[%d]:0x%x"
			" ?= result[%d]:0x%x\n",
...
static void fastcg_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < 7; i++) {
		...
		if ((i == 2) && (data != 0x50) && (!fw_ver_info)) {
			//data received not starting with "101"
			pr_err("%s data err:%d\n", __func__, data);
	...
	if (data == 0x52) {
		...
		//fastchg full,vbatt > 4350, switch off fast chg
		...
		if ((!pic_have_updated) &&
		    (Pic16F_firmware_data[pic_fw_ver_count - 4] > data)) {
			ret_info = 0x2;
