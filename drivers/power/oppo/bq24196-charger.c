...
struct bq24196_access_methods {
	int (*read) (struct bq24196_device_info * di, u8 reg, u8 length,
		     char *buf);
	int (*write) (struct bq24196_device_info * di, u8 reg, u8 length,
		      char *buf);
};
struct bq24196_device_info {
	struct device *dev;
...
static int bq24196_read_i2c(struct bq24196_device_info *di, u8 reg, u8 length,
			    char *buf)
{
...
static int bq24196_write_i2c(struct bq24196_device_info *di, u8 reg, u8 length,
			     char *buf)
{
	struct i2c_client *client = di->client;
	int retval;
...
static int bq24196_read(struct bq24196_device_info *di, u8 reg, u8 length,
			char *buf)
{
	return di->bus->read(di, reg, length, buf);
}

static int bq24196_write(struct bq24196_device_info *di, u8 reg, u8 length,
			 char *buf)
{
...
static int bq24196_ibatmax_set(struct bq24196_device_info *di, int chg_current)
{
	u8 value = 0;
...
bq24196_ibatterm_set(struct bq24196_device_info *di, int term_current)
{
	...
	if ((term_current < BQ24196_TERM_CURR_MIN)
	    || (term_current > BQ24196_TERM_CURR_MAX)) {
	...
	return bq24196_chg_masked_write(di, PRECHARGE_TERMCURR_CTRL,
					BQ24196_IBATTERM_BITS, value, 1);
}

#define BQ24196_IUSBMAX_BITS		0X07
static int bq24196_iusbmax_set(struct bq24196_device_info *di, int mA)
{
	u8 value = 0;
	pr_info("%s mA:%d\n", __func__, mA);
	...
	if (mA == BQ24196_CHG_IUSBMAX_MAX)
		...
		value = 0x0;
	return bq24196_chg_masked_write(di, INPUT_SOURCE_CTRL,
					BQ24196_IUSBMAX_BITS, value, 1);
}

#define BQ24196_VBATDET_BITS		0x1
static int bq24196_vbatdet_set(struct bq24196_device_info *di, int vbatdet)
{
	u8 value = 0;
	...
	return bq24196_chg_masked_write(di, CHARGE_VOL_CTRL,
					BQ24196_VBATDET_BITS, value, 1);
}

#define BQ24196_CHG_VDDMAX_MIN		3504
#define BQ24196_CHG_VDDMAX_MAX		4400
#define BQ24196_VDDMAX_BITS		0xFC
static int bq24196_vddmax_set(struct bq24196_device_info *di, int voltage)
{
	u8 value = 0;
	pr_info("%s voltage:%d\n", __func__, voltage);
	...
	return bq24196_chg_masked_write(di, CHARGE_VOL_CTRL,
					BQ24196_VDDMAX_BITS, value, 1);
}

#define BQ24196_VINMIN_MIN		3880
#define BQ24196_VINMIN_MAX		5080
#define BQ24196_VINMIN_BITS		0x78
static int bq24196_vinmin_set(struct bq24196_device_info *di, int voltage)
{
	u8 value = 0;
	pr_info("%s voltage:%d\n", __func__, voltage);
	if (voltage < BQ24196_VINMIN_MIN || voltage > BQ24196_VINMIN_MAX) {
	...
	value <<= 3;
	return bq24196_chg_masked_write(di, INPUT_SOURCE_CTRL,
					BQ24196_VINMIN_BITS, value, 1);	//default 4.36v
}
...
bq24196_check_charge_timeout(struct bq24196_device_info *di, int hours)
{
	...
	if (hours >= 20)	//20 hours
		...
		value = 0x0;
	return bq24196_chg_masked_write(di, CHARGE_TERM_TIMER_CTRL,
					BQ24196_CHARGE_TIMEOUT_BITS, value, 1);
}
...
bq24196_otg_current_set(struct bq24196_device_info *di, int otg_current)
{
	...
	if (otg_current >= 1000)
		...
		value = 0x0;	//500ma
	return bq24196_chg_masked_write(di, POWER_ON_CONF,
					BQ24196_OTG_CURRENT_BITS, value, 1);
}

#define BQ24196_WDT_SET_BITS		0x30
static int bq24196_wdt_set(struct bq24196_device_info *di, int seconds)
{
	u8 value = 0;
	...
	return bq24196_chg_masked_write(di, CHARGE_TERM_TIMER_CTRL,
					BQ24196_WDT_SET_BITS, value, 1);
}

#define BQ24196_REGS_RESET_BITS		0x80
static int bq24196_regs_reset(struct bq24196_device_info *di, int reset)
{
	u8 value = 0;
	if (reset)		// 1 is reset
		value = 0x80;
		...
		value = 0x0;
	return bq24196_chg_masked_write(di, POWER_ON_CONF,
					BQ24196_REGS_RESET_BITS, value, 1);
}

#define BQ24196_CHARGEEN_BITS		0x30
static int bq24196_charge_en(struct bq24196_device_info *di, int enable)
{
	u8 value = 0;
	pr_info("%s enable:%d\n", __func__, enable);
	...
	if (enable == 1)	//enable charge
		...
		value = 0x20;
	return bq24196_chg_masked_write(di, POWER_ON_CONF,
					BQ24196_CHARGEEN_BITS, value, 1);
}

static int bq24196_get_charge_en(struct bq24196_device_info *di)
{
	char value_buf;
	int rc;
...
bq24196_usb_suspend_enable(struct bq24196_device_info *di, int enable)
{
	...
	if (enable)
		...
		value = 0x0;
	return bq24196_chg_masked_write(di, INPUT_SOURCE_CTRL,
					BQ24196_USB_SUSPEND_ENABLE_BITS, value,
					1);
}
