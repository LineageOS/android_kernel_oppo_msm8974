...
t[5]:T4
 */
#define SLED_SET_BREATHTIME	_IOW(SLED_IOCTL_MAGIC, 4, unsigned)	//set breateh time T0~T4
/*
config register 05h
*/
...
static int SN3193_write_reg(u16 reg, u8 val)
{
	...
	if (i2c_master_send(SN3193_sled_dev_sate->gi2c_dev, buf, 2) != 2) {
		dev_err(&SN3193_sled_dev_sate->gi2c_dev->dev,
			"%s: i2c send failed\n", __func__);
		return -EIO;
	}
				...
				size_t count)
{
	...
	if (~(~value) && totalMS && onMS) {
		...
		if (shine_debug) {
			printk
			    ("shineled----%s:   color_R = %d: color_G = %d: color_B = %d,\n",
				...
				enum led_brightness value)
{
	...
	if (!strcmp(led_cdev->name, "blue")) {
		if (shine_debug)
			printk
			    ("shineled----%s: light the blue ,value = %d\n",
			     __func__, value);
		color_B = value;
		brightness_time++;
	}
			...
			const struct i2c_device_id *id)
{
	...
	for (i = 0; i < 3; i++) {
		if (led_classdev_register(&client->dev, &SN3193_lcds[i])) {
			printk(KERN_ERR
			       "led_classdev_register failed of SN3193_lcds!\n");
