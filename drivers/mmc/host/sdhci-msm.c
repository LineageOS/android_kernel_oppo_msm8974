...
retry:
	...
	if (tuned_phase_cnt) {
        ...
        if(mmc_mid == 0x15)  //songxh modified phase value
		{
			if(phase != 6 && phase != 7)
			{
				if(tuned_phase_cnt >= 6)
				{				
					printk("force phase to %d for samsung emmc\n", tuned_phases[6]);
					phase = tuned_phases[6];
				}
				else
				{
					phase = tuned_phases[tuned_phase_cnt];
					printk("force phase to tuned_phases[%d] %d for samsung emmc\n", tuned_phase_cnt, tuned_phases[tuned_phase_cnt]);
				}
			}
		}
#endif
